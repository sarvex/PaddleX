    ...
    if classwise:
        ...
        for idx, catId in enumerate(cat_ids):
            ...
            draw_pr_curve(
                pr_array,
                recall_array,
                out_dir=f'{style}_pr_curve',
                file_name=f'{nm["name"]}_precision_recall_curve.jpg',
            )
        ...
        logging.info(f'Per-category of {style} AP: \n{table.table}')
        logging.info(f"per-category PR curve has output to {style}_pr_curve folder.")
...
def loadRes(coco_obj, anns):
    ...
    import time
    res = COCO()
    res.dataset['images'] = list(coco_obj.dataset['images'])
