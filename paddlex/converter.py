...
class MultiClassNMS4OpenVINO():
    ...
    def opset_10(cls, graph, node, **kw):
        ...
        node_const_value = [
            f"{result_name}@const_0",
            f"{result_name}@const_1",
            f"{result_name}@const_2",
            f"{result_name}@const_-1",
        ]
        ...
        node_gather_1 = graph.make_node(
            'Gather', inputs=[node_select_nms, f"{result_name}@const_1"], axis=1
        )
        ...
        node_gather_2 = graph.make_node(
            'Gather', inputs=[node_select_nms, f"{result_name}@const_2"], axis=1
        )
        ...
        node_reshape_scores_rank1 = graph.make_node(
            "Reshape", inputs=[node.input('Scores', 0), f"{result_name}@const_-1"]
        )
        ...
        node_gather_scores_dim1 = graph.make_node(
            'Gather', inputs=[node_shape_scores, f"{result_name}@const_2"], axis=0
        )
        ...
        node_gather_select_num = graph.make_node(
            'Gather',
            inputs=[node_shape_select_num, f"{result_name}@const_0"],
            axis=0,
        )
        ...
        outputs_topk_select_topk_indices = [
            f"{result_name}@topk_select_topk_values",
            f"{result_name}@topk_select_topk_indices",
        ]
        ...
        outputs_topk_select_classes_indices = [
            f"{result_name}@topk_select_topk_classes_scores",
            f"{result_name}@topk_select_topk_classes_indices",
        ]
        node_topk_select_topk_indices = graph.make_node(
            'TopK',
...
def export_onnx_model(model, save_file, opset_version=10):
    ...
    if p2o.__version__ != '0.4':
        logging.error(
            f"You need install paddle2onnx==0.4, but the version of paddle2onnx is {p2o.__version__}"
        )
